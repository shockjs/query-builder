...
class QueryBuilder {
  ...
  addPrimaryKey(name, table, columns) {
    ...
    return 'ALTER TABLE ' + this._db.quoteTableName(table) + ' ADD CONSTRAINT ' + this._db.quoteColumnName(name) + '  PRIMARY KEY (' + columns.join(', ') + ' )';
  }
  ...
  dropPrimaryKey(name, table) {
    return 'ALTER TABLE ' + this._db.quoteTableName(table) + ' DROP CONSTRAINT ' + this._db.quoteColumnName(name);
  }
  ...
  addColumn(table, column, type) {
    return 'ALTER TABLE ' + this._db.quoteTableName(table) + ' ADD ' + this._db.quoteColumnName(column) + ' ' + this.getColumnType(type);
  }
  ...
  dropColumn(table, column) {
    return "ALTER TABLE " + this._db.quoteTableName(table) + " DROP COLUMN " + this._db.quoteColumnName(column);
  }
  ...
  renameColumn(table, oldName, newName, type = null) {
    return "ALTER TABLE " + this._db.quoteTableName(table) + " RENAME COLUMN " + this._db.quoteColumnName(oldName) + " TO " + this._db.quoteColumnName(newName);
  }
  ...
  alterColumn(table, column, type) {
    return 'ALTER TABLE ' + this._db.quoteTableName(table) + ' CHANGE ' + this._db.quoteColumnName(column) + ' ' + this._db.quoteColumnName(column) + ' ' + this.getColumnType(type);
  }
  ...
  addForeignKey(name, table, columns, refTable, refColumns, remove = null, update = null) {
    var sql = 'ALTER TABLE ' + this._db.quoteTableName(table) + ' ADD CONSTRAINT ' + this._db.quoteColumnName(name) + ' FOREIGN KEY (' + this.buildColumns(columns) + ')' + ' REFERENCES ' + this._db.quoteTableName(refTable) + ' (' + this.buildColumns(refColumns) + ')';
    if (remove !== null) {
  ...
  createIndex(name, table, columns, unique = false) {
    return (unique ? 'CREATE UNIQUE INDEX ' : 'CREATE INDEX ') + this._db.quoteTableName(name) + ' ON ' + this._db.quoteTableName(table) + ' (' + this.buildColumns(columns) + ')';
  }
  ...
  buildSelect(columns, params, distinct, selectOption) {
    ...
    columns = _lodash2.default.isArray(columns) ? columns : Object.keys(columns).map(key => columns[key]);
  ...
  buildFrom(tables, params) {
    ...
    tables = _lodash2.default.isArray(tables) ? tables : Object.keys(tables).map(key => tables[key]);
  ...
  buildOrderBy(columns) {
    ...
    var orders = [];
    Object.keys(columns).forEach(name => {
      orders.push(this._db.quoteColumnName(name) + (columns[name] === _constants.SORT_DESC ? ' DESC' : ''));
    });
