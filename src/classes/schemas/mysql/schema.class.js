import {TableSchema} from "../../table.schema.class";
import {Schema as BaseSchema} from "../../schema.class";
import {ColumnSchema} from "../../column.schema.class";
...
export class Schema extends BaseSchema {
  ...
  _findColumns(table) {
    ...
    return this._db.createCommand(sql)
      .execute()
      .then(({rows: columns}) => {
        columns.forEach((info) => {
          info = _.transform(info, function (result, val, key) {
  ...
  {
    return this._db.createCommand('SHOW CREATE TABLE ' + this._db.quoteTableName(table.fullName))
      .execute()
      .then(({rows}) =>{
        var sql;
